...
///      homomorphic additions the aggregate error has sigma ~ 2.83 * sqrt(M).
///      Decoding is correct while |error| < Delta/2 = 32768, so with a 7.5-sigma
///      margin (failure < 2^-40) the budget allows M ~ 2.3 million additions.
///      The binding constraint is plaintext capacity: the aggregate must stay < p.
///
...
library RegevParameters {
    ...
    uint256 internal constant Q_MASK = 0xFFFFFFFF; // Q - 1
    uint256 internal constant BIT_WIDTH = 32;
    uint256 internal constant ELEMENTS_PER_WORD = 8;
    uint256 internal constant PACKED_WORDS = 192; // N / ELEMENTS_PER_WORD

    // Plaintext space
    uint256 internal constant P = 1 << 16;
    uint256 internal constant P_MASK = 0xFFFF;
    uint256 internal constant DELTA = 1 << 16; // Q / P
    uint256 internal constant DELTA_SHIFT = 16; // log2(DELTA)

    // Centered binomial noise parameter: e = popcount(x[0..15]) - popcount(x[16..31])
    // k = 16 gives sigma = sqrt(k/2) ~ 2.83, |e| <= 16
